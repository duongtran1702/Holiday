...
export function ProductCard({ product, onAdd, onClick }: { product: ProductDto; onAdd: (item: any) => void; onClick?: (p: ProductDto) => void }) {
  ...
  return (
    <div className="group bg-card border border-border rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
      <div className="relative overflow-hidden aspect-[3/4] bg-muted cursor-pointer" onClick={() => onClick && onClick(product)}>
      ...
      <div className="p-3.5">
        ...
        <h3 style={{ fontFamily: "'Playfair Display', serif" }} className="font-semibold text-sm leading-snug mb-1.5 cursor-pointer hover:text-accent transition-colors" onClick={() => onClick && onClick(product)}>{product.name}</h3>
